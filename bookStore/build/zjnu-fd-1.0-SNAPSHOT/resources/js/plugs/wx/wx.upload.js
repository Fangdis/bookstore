 ...
 * wx.upload
 * 
 * 文件上传组件 支持三种上传方式，html5、flash和iframe 目的是以简单并且通用的方式解决上传问题
 * 
...
(function(window, document, $, wx, undefined) {
	...
	var W3C = window.FormData !== undefined ? true : false, prefix = "wx-upload";
	...
	function init($elem) {
		...
		options.url += options.url.indexOf("?") !== -1 ? "&_="
				+ new Date().getTime() : "?_=" + new Date().getTime();
		options.size = options.size ? parseFloat(options.size) * 1024 * 1024
				: null;
		options.callback = $.isFunction(window[options.name]) ? window[options.name]
				: null;
		options.before = $.isFunction(window[options.name + "_before"]) ? window[options.name
				+ "_before"]
				: null;
		options.progress = $.isFunction(window[options.name + "_progress"]) ? window[options.name
				+ "_progress"]
				: null;
	...
	function complete(responseText, options, $input) {
		...
		try {
			...
			if (options.assign
					&& data[wx.config.dataFlag] == wx.config.dataSuccessVal) {
				var assign = options.assign.split("&");
				for (var i = 0; i < assign.length; i++) {
					var assVal = "", assignItem = assign[i].split("="), $inputAss = $("input[name='"
							+ assignItem[0] + "']");
					assVal = eval('data["'
							+ assignItem[1].replace(/\./g, '"]["') + '"]');
					if ($inputAss.length)
						$inputAss.val(assVal);
					else
						$input.before('<input name="' + assignItem[0]
								+ '" value="' + assVal
								+ '" style="display:none;">');
				}
			}
			if (options.set
				...
				for (var i = 0; i < set.length; i++) {
					var setVal = "", setItem = set[i].split("="), $elem = $("#"
							+ setItem[0]);
					if ($elem.length) {
						setVal = eval('data["'
								+ setItem[1].replace(/\./g, '"]["') + '"]');
						setVal += setVal.indexOf("?") !== -1 ? "&_"
								+ new Date().getTime() : "?_="
								+ new Date().getTime();
						if ($elem.is("img"))
			...
			//wx.log("uploadComplete error " + e);
		}
	}

	function h5() {
		var fd = null, xhr = null, files = this.files, $input = $(this), options = $input
				.data("opt");

		if (!before(options, $input))
			return;
		...
		for (var i = 0; i < files.length; i++) {
			if (options.size && files[i].size > options.size) {
				...
				continue;
					...
					&& options.type !== "*"
					&& (!files[i].type || options.type.indexOf(files[i].type
							.split("/")[1]) === -1)) {
				...
				wx.alert("文件格式不符，<br />允许上传的格式为" + type);
				continue;
			}
		...
		function bindEvent(xhr) {
			...
			xhr.addEventListener("load", function(evt) {
				complete(evt.target.responseText, options, $input);
			}, false);
			xhr.addEventListener("error",
					function(error) {
						complete('{"status:0",error:"' + error + '"}', options,
								$input);
					}, false);
		}
	}
	...
	function flash($input) {
		var width = $input.width(), height = $input.height(), options = $input
				.data("opt"), flashOpt = {};
		...
		var falshObj = wx.loadFlash("wx", {
			id : "wx-falsh",
			"width" : width,
			"height" : height
		}, {
			flashvars : wx.jsonToString(flashOpt)
		});
		$input
				.before($(
						'<div style="position:absolute;cursor:pointer;opacity:0;z-index:0;overflow:hidden;width:'
								+ width
								+ 'px;height:'
								+ height
								+ 'px;top:'
								+ $input.position().top
								+ 'px;left:'
								+ $input.position().left + '"></div>').append(
						falshObj));
		$input.remove();
	...
	function normal() {
		var $iframe = null, $input = $(this), $inputC = $input.clone(), options = $input
				.data("opt"), $form = $('<form id="wx-upload-form" method="post" action="'
				+ options.url
				+ '" enctype="multipart/form-data" target="wx-upload-iframe"></form>');
		...
		if (wx.browser.msie && wx.browser.version == 6) {
			var io = document
					.createElement('<iframe id="wx-upload-iframe" name="wx-upload-iframe" />');
		...
		$form.appendTo('body');
		if (options.param) {
			var tempURL = options.param.split('&'), param = "";
			for (var i = 0; i < tempURL.length; i++) {
				var t = tempURL[i].split('=');
				param += '<input name="' + t[0] + '" value="' + t[1]
						+ '" style="display:none;">';
			}
			$form.append(param);
		}
		$iframe
				.on(
						"load",
						function() {
							var content = this.contentWindow ? this.contentWindow
									: this.contentDocument, reponse = content.document.body ? content.document.body.innerHTML
									: null;
