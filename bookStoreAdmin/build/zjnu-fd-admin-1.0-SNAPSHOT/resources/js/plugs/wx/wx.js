    ...
    function initElement(){
      ...
      formInfo.$select.each(function(){
        var $thisSelect = $(this);
        	...
        	if(name!=undefined)
        		name=name.replace(".","\\.");
        	  var  thisAttr    = {"va" : $thisSelect.attr(prefix+"-error-value"),
                           "me" : prefix+"-"+name+"-error",
                           "st" : $thisSelect.attr(prefix+"-show-type") || "normal",
                           "su" : $("#"+prefix+"-"+name+"-success",$thisForm),
        ...
        if(thisAttr["ph"]){
          if('placeholder' in this){
            ...
            var $inputNotice = $thisInput.next('.wx-placeholder').length ?
                               ...
                               $('<span class="wx-placeholder" style="font-size:inherit;color:#ccc;position:absolute;width:'+($thisInput.width()-10)+'px;top:'+($thisInput.position().top)+'px;left:'+($thisInput.position().left+5)+'px">'+thisAttr["ph"]+'</span>');
                               console.log($thisInput.position().left);
  ...
  */
  ...
  wx.validator.rule = {
    ...
    password: function(value) {
        return value.length === 0 || /^(?=.*[a-zA-Z]+)(?=.*[0-9]+)[a-zA-Z0-9]{6,16}$/.test(value);
    },
    code: function(value) {
        return value.length === 0 || /^.{4}$/.test($.trim(value));
    }
  };
  /**
   * 懒加载
